...
export const ComEditor: React.FC<IProps> = (props) => {
  ...
  const getUploadImgUrl = async (formData: FormData) => {
    try {
      const res: any = await fetchUploadImg(formData)
  ...
  const submitHandle = async () => {
    ...
    const res = await fetchSubmitArticle({userId, content: html, categoryList: selectedTags, title: titleValue, coverUrl: imageUrl});
        ...
        <Form form={form}>
          ...
          <Form.Item label="文章封面" name="articleCover">
            ...
            //  className="avatar-uploader"
             ...
             action="http://localhost:8008/article/img/upload"
